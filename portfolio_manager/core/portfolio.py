...
class Portfolio:
    ...
    def __init__(self, db_path: str = None):
        ...
        self._last_market_refresh: Optional[datetime] = self.db.get_last_market_refresh()
    ...
    def refresh_market_data(self) -> int:
        ...
        positions = self.db.get_current_positions()
        tickers = [p.ticker for p in positions]
        count = self.market_api.refresh_all_cache(tickers)
        self._last_market_refresh = datetime.now()
        return count

    def get_last_market_refresh_time(self, reload: bool = False) -> Optional[datetime]:
        """Retourne le timestamp de la dernière mise à jour du cache marché."""
        if reload or self._last_market_refresh is None:
            self._last_market_refresh = self.db.get_last_market_refresh()
        return self._last_market_refresh

    def needs_market_refresh_today(self) -> bool:
        """Indique si une mise à jour des prix est nécessaire pour la journée."""
        last_refresh = self.get_last_market_refresh_time()

        if not last_refresh:
            return True

        return last_refresh.date() < datetime.now().date()

    def refresh_market_data_if_needed(self, force: bool = False) -> int:
        """Met à jour les prix uniquement si nécessaire (ou forcé)."""
        if not force and not self.needs_market_refresh_today():
            return 0
