...
class DatabaseManager:
    ...
    def get_last_market_refresh(self) -> Optional[datetime]:
        """Retourne la dernière date de mise à jour du cache marché."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(Queries.GET_LAST_MARKET_REFRESH)
            row = cursor.fetchone()

            if not row:
                return None

            if hasattr(row, 'keys') and 'last_updated' in row.keys():
